...
fn main() {
  ...
  println!("Test:   {}", check_result(result, 8, duration));

  let start = Instant::now();
  let result = part_1::part_1(INPUT);
  let duration = start.elapsed();
  println!("Answer: {}", check_result(result, 2600, duration));
  ...
  let duration = start.elapsed();
  println!("Test:   {}", check_result(result, 2286, duration));

  let start = Instant::now();
  let result = part_2::part_2(INPUT);
  let duration = start.elapsed();
  println!("Answer: {}", check_result(result, 86036, duration));
}
